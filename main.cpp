...
cl_mem 				universe_buffer, update_buffer;
...
void touch() {
	...
	clEnqueueAcquireGLObjects(queue, 2, buffers, 0, NULL, NULL);
    clEnqueueReadImage(queue, universe_buffer, CL_TRUE, origin, region, 0, 0, &seed, 0, NULL, NULL);

    if(seed < 128) { seed = 0xff; }
    else { seed = 0x00; }
...
void initDisplay(unsigned int width, unsigned int height) {
	...
	if(!glfwOpenWindow(width, height, 0, 0, 0, 0, 32, 0, GLFW_WINDOW)) {
		...
		glfwSetWindowTitle("CL Game of Life");
...
void initCL() {
	...
	kernel = loadKernel(context, device, "clgol_kernel.cl", build_options.str().data(), "gol");
...
int main() {
	...
	// Create some universe seed
	std::cout << "= Pupulate universe" << std::endl;
    const size_t origin[] = {5, 10, 0};
    const size_t region[] = {3, 3, 1};

	clEnqueueAcquireGLObjects(queue, 1, &universe_buffer, 0, NULL, NULL);
    clEnqueueWriteImage(queue, universe_buffer, CL_TRUE, origin, region, 0, 0, glider, 0, NULL, NULL);
	clEnqueueReleaseGLObjects(queue, 1, &universe_buffer, 0, NULL, NULL);
	clFinish(queue);
